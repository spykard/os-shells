...
int main(int argc, char *argv[])
{
	...
	int i, cc, flag, status, pid[2], fd[2];
	...
	{	printf("$"); // PROMPT
		...
		sInput = strtok(usrInput, " \n"); // Parse user input
	 	...
	 	for (i = 0; prog_argv[0][i] != NULL; i++){ // Split commands at |
			if (!strcmp(prog_argv[0][i], "|")) {
				flag=1;                // Input contains pipe
				prog_argv[0][i] = NULL;
				while (prog_argv[0][i+1] != NULL) {
					prog_argv[1][cc] = prog_argv[0][i+1];
					prog_argv[0][i+1] = NULL;
					i++;
					cc++;}
				}
				prog_argv[1][cc] = NULL;}
		
		if (flag == 1) // Input with 1 pipe
		{	if (pipe(fd)<0)
			{	perror("Pipe error");
	        	exit(98);}
